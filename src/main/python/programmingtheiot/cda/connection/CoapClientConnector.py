	...
	def startObserver(self, resource: ResourceNameEnum = None, name: str = None, ttl: int = IRequestResponseClient.DEFAULT_TTL) -> bool:
		if resource or name:
			if resource in self.observeRequests:
				logging.warning("Already observing resource %s. Ignoring start observe request.", str(resource))
				return
	...
	def stopObserver(self, resource: ResourceNameEnum = None, name: str = None, timeout: int = IRequestResponseClient.DEFAULT_TIMEOUT) -> bool:
		if resource or name:
			if not resource in self.observeRequests:
				logging.warning("Resource %s not being observed. Ignoring stop observe request.", str(resource))
				return
			
			response = self.observeRequests[resource]
			
			if response:
				logging.info("Canceling observe for resource %s.", str(resource))
				...
				try:
					...
					logging.info("Canceled observe for resource %s.", str(resource))
				except Exception as e:
					logging.warning("Failed to cancel observe for resource %s.", str(resource))
			else:
				logging.warning("No response yet for observed resource %s. Attempting to stop anyway.", str(resource))
